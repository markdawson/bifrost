...
export default function VirtualMCPsTable({
	...
	const { data: mcpClientsData, isLoading: clientsLoading } = useGetMCPClientsQuery({ limit: 1000 });
	...
	const toolWarning = (row: VirtualMCP): string | null => {
		// Until the client list loads, clientById is empty; don't flag every source as unreachable.
									...
									<TableRow key={row.id} className="group">
										...
										<TableCell className="text-muted-foreground text-sm">
											<span className="flex items-center gap-1.5">
												{clientsLoading ? "…" : includedToolCount(row)}
												{warning && (
													<Tooltip>
...
function RowActions({ busy, onEdit, onDelete }: { busy: boolean; onEdit: () => void; onDelete: () => void }) {
	return (
		<DropdownMenu>
