...
- **Both fields are required.** A multiplier with no schedule has no way to know when to apply, and a schedule with no multiplier has nothing to apply. Either one alone bills at the base rate.
- **Flat fees are not discounted.** `cost_per_request` and `search_context_cost_per_query` are per-request and per-query charges rather than usage charges, so they are billed in full at any hour.
